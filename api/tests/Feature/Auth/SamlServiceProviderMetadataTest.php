<?php

declare(strict_types=1);

namespace Tests\Feature\Auth;

use PHPUnit\Framework\Attributes\Test;
use Tests\TestCase;

final class SamlServiceProviderMetadataTest extends TestCase
{
    #[Test]
    public function it_serves_service_provider_metadata_xml(): void
    {
        config()->set('app.url', 'https://phpgrc.example');
        config()->set('saml.sp', array_merge(config('saml.sp', []), [
            'entityId' => 'urn:phpgrc:sp',
            'assertionConsumerService' => array_merge(
                config('saml.sp.assertionConsumerService', []),
                [
                    'url' => 'https://phpgrc.example/auth/saml/acs',
                ]
            ),
            'metadataUrl' => 'https://phpgrc.example/auth/saml/metadata',
        ]));

        config()->set('saml.security', array_merge(config('saml.security', []), [
            'authnRequestsSigned' => false,
            'wantAssertionsSigned' => true,
            'wantAssertionsEncrypted' => false,
        ]));

        $response = $this->get('/auth/saml/metadata');

        $response->assertStatus(200);
        self::assertSame('application/samlmetadata+xml; charset=UTF-8', $response->headers->get('Content-Type'));
        self::assertSame('nosniff', $response->headers->get('X-Content-Type-Options'));

        $xml = $response->getContent();
        self::assertIsString($xml);

        $document = simplexml_load_string($xml);
        self::assertInstanceOf(\SimpleXMLElement::class, $document);
        $document->registerXPathNamespace('md', 'urn:oasis:names:tc:SAML:2.0:metadata');

        $entityDescriptor = $document->xpath('/md:EntityDescriptor');
        self::assertNotFalse($entityDescriptor);
        self::assertNotEmpty($entityDescriptor);
        self::assertSame('urn:phpgrc:sp', (string) ($entityDescriptor[0]['entityID'] ?? ''));

        $acsNodes = $document->xpath('/md:EntityDescriptor/md:SPSSODescriptor/md:AssertionConsumerService');
        self::assertNotFalse($acsNodes);
        self::assertNotEmpty($acsNodes);
        self::assertSame('https://phpgrc.example/auth/saml/acs', (string) ($acsNodes[0]['Location'] ?? ''));

        $spDescriptor = $document->xpath('/md:EntityDescriptor/md:SPSSODescriptor');
        self::assertNotFalse($spDescriptor);
        self::assertNotEmpty($spDescriptor);
        self::assertSame('false', (string) ($spDescriptor[0]['AuthnRequestsSigned'] ?? ''));
        self::assertSame('true', (string) ($spDescriptor[0]['WantAssertionsSigned'] ?? ''));
        $signingNodes = $document->xpath('/md:EntityDescriptor/md:SPSSODescriptor/md:KeyDescriptor[@use="signing"]');
        self::assertNotFalse($signingNodes);
        self::assertCount(0, $signingNodes);
        $encryptionNodes = $document->xpath('/md:EntityDescriptor/md:SPSSODescriptor/md:KeyDescriptor[@use="encryption"]');
        self::assertNotFalse($encryptionNodes);
        self::assertCount(0, $encryptionNodes);
    }

    #[Test]
    public function it_includes_certificate_when_configured(): void
    {
        config()->set('app.url', 'https://phpgrc.example');
        config()->set('saml.sp', array_merge(config('saml.sp', []), [
            'entityId' => 'urn:phpgrc:sp:cert',
            'assertionConsumerService' => array_merge(
                config('saml.sp.assertionConsumerService', []),
                [
                    'url' => 'https://phpgrc.example/auth/saml/acs',
                ]
            ),
            'metadataUrl' => 'https://phpgrc.example/auth/saml/metadata',
            'x509cert' => $this->sampleCertificate(),
        ]));

        config()->set('saml.security', array_merge(config('saml.security', []), [
            'authnRequestsSigned' => true,
            'wantAssertionsSigned' => false,
            'wantAssertionsEncrypted' => true,
        ]));

        $response = $this->get('/auth/saml/metadata');

        $response->assertStatus(200);

        $xml = $response->getContent();
        self::assertIsString($xml);

        $document = simplexml_load_string($xml);
        self::assertInstanceOf(\SimpleXMLElement::class, $document);
        $document->registerXPathNamespace('md', 'urn:oasis:names:tc:SAML:2.0:metadata');
        $document->registerXPathNamespace('ds', 'http://www.w3.org/2000/09/xmldsig#');

        $certNodes = $document->xpath('/md:EntityDescriptor/md:SPSSODescriptor/md:KeyDescriptor[@use="signing"]/ds:KeyInfo/ds:X509Data/ds:X509Certificate');
        self::assertNotFalse($certNodes);
        self::assertNotEmpty($certNodes);
        self::assertStringContainsString('MIICxjCCAa6gAwIBAgIUJ7X7YvXy5whh', (string) $certNodes[0]);

        $spDescriptor = $document->xpath('/md:EntityDescriptor/md:SPSSODescriptor');
        self::assertNotFalse($spDescriptor);
        self::assertNotEmpty($spDescriptor);
        self::assertSame('true', (string) ($spDescriptor[0]['AuthnRequestsSigned'] ?? ''));
        self::assertSame('false', (string) ($spDescriptor[0]['WantAssertionsSigned'] ?? ''));
        $encryptionDescriptors = $document->xpath('/md:EntityDescriptor/md:SPSSODescriptor/md:KeyDescriptor[@use="encryption"]');
        self::assertNotFalse($encryptionDescriptors);
        self::assertCount(1, $encryptionDescriptors);
    }

    private function sampleCertificate(): string
    {
        return <<<'PEM'
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
PEM;
    }
}
